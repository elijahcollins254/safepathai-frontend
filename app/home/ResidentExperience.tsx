...
export default function ResidentExperience({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  useEffect(() => {
    if ((window as Window & { google?: GoogleMapsApi }).google) setMapsReady(true);
  }, []);
  ...
  return (
    <main className="resident-shell">
      {(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY) && <Script src={`https://maps.googleapis.com/maps/api/js?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY}`} strategy="afterInteractive" onLoad={() => setMapsReady(true)} onReady={() => setMapsReady(true)} />}
      <button className="resident-profile-trigger" type="button" aria-label="Open your details" onClick={() => setProfileOpen(true)}>Your details</button>
