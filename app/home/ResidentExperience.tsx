...
export default function ResidentExperience({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  return (
    <main className="resident-shell">
      ...
      {(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY) && <Script src={`https://maps.googleapis.com/maps/api/js?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY}&libraries=places&loading=async`} strategy="afterInteractive" onLoad={() => setMapsReady(true)} />}
