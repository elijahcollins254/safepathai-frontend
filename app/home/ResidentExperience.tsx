...
function ResidentMapSurface({ location, hazards, shelters, zones, selectedRoute, ready, searchRequest, searchQuery, searchPoint, zoomRequest, streetViewRequest, onSearchResult, onSearchError }: { location: LatLng | null; hazards: Hazard[]; shelters: Shelter[]; zones: Zone[]; selectedRoute: ResidentRoute | null; ready: boolean; searchRequest: number; searchQuery: string; searchPoint: LatLng | null; zoomRequest: number; streetViewRequest: number; onSearchResult: (point: LatLng) => void; onSearchError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!mapsApi || typeof mapsApi.maps?.Map !== "function" || !mapElement.current || !ready) return;
...
export default function ResidentExperience({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  const [visibleLayers, setVisibleLayers] = useState({ hazards: true, shelters: true, zones: true });
  ...
  async function findRoutes() {
    ...
    setRouteMessage("");
    try {
      const response = await fetch(`${apiBaseUrl}/route/recommend/`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ latitude: location.lat, longitude: location.lng, travel_mode: travelMode }) });
  ...
  }, [hazards, zones, location, travelMode]);

  const currentHazardZone = location ? zones.find((zone) => zone.zone_type === "hazard" && pointInPolygon(location, zone.coordinates)) : null;
  const nearbyHazards = location ? hazards.filter((hazard) => distanceBetweenPoints(location, { lat: hazard.latitude, lng: hazard.longitude }) <= hazard.radius) : [];
  const hazardAlert = Boolean(currentHazardZone || nearbyHazards.length);
  const visibleHazards = visibleLayers.hazards ? hazards : [];
  const visibleShelters = visibleLayers.shelters ? shelters : [];
  const visibleZones = visibleLayers.zones ? zones : [];
  ...
  return (
    <main className="resident-shell">
      {hazardAlert && <div className="resident-hazard-alert" role="alert"><span className="resident-hazard-alert-icon">!</span><div><strong>{currentHazardZone ? `${currentHazardZone.name} is a hazard zone` : "Hazard nearby"}</strong><p>Move away from the marked area and follow official safety instructions.</p></div></div>}
      <button className="resident-owner-button" type="button" aria-label="Do you own this website? Open operator access" title="Do you own this website?" onClick={() => router.push("/admin")}><span>⌂</span><small>Do you own this website?</small></button>
      {(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY) && <Script src={`https://maps.googleapis.com/maps/api/js?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY}&libraries=places&loading=async`} strategy="afterInteractive" onLoad={() => {
        const waitForMaps = () => {
          const mapsApi = (window as Window & { google?: GoogleMapsApi }).google;
      ...
      <div className="resident-map-panel"><form className="resident-map-searchbar" onSubmit={(event) => { event.preventDefault(); if (searchQuery.trim()) setSearchRequest((current) => current + 1); }}><button className="resident-map-menu" type="button" aria-label="Open map menu">☰</button><input ref={searchInputRef} aria-label="Search places, addresses, or businesses" placeholder="Search places, addresses, or businesses..." value={searchQuery} onChange={(event) => { setSearchQuery(event.target.value); setSelectedPlace(null); }} /><button className="resident-map-search" type="submit" aria-label="Search map">⌕</button></form><nav className="resident-map-quick-actions" aria-label="Safety actions"><button type="button" className="danger-action" aria-label="Call emergency services" title="Call emergency services" onClick={() => { window.location.href = "tel:112"; }}><span>!</span><small>Emergency</small></button><button type="button" aria-label="Open your details" title="Your details" onClick={() => setProfileOpen(true)}><span>+</span><small>My details</small></button><button type="button" aria-label="Open calm guide" title="Calm guide" onClick={() => setChatOpen(true)}><span>✦</span><small>Calm guide</small></button></nav><div className="resident-map-tools"><button type="button" aria-label="Show map layers" title="Map layers" onClick={() => setLayersOpen((open) => !open)}><span>◈</span><small>Layers</small></button><button type="button" aria-label="Center on my location" title="My location" onClick={shareLocation}><span>◎</span><small>My location</small></button><button type="button" aria-label="Open Street View" title="Street view" onClick={() => location ? setStreetViewRequest((current) => current + 1) : setRouteMessage("Share your location first to open Street View.")}><span>◉</span><small>Street view</small></button><button type="button" aria-label="Zoom in" title="Zoom in" onClick={() => setZoomRequest((current) => current + 1)}><span>+</span><small>Zoom in</small></button><button type="button" aria-label="Zoom out" title="Zoom out" onClick={() => setZoomRequest((current) => current - 1)}><span>−</span><small>Zoom out</small></button></div>{layersOpen && <div className="resident-layer-panel" role="group" aria-label="Map layers"><strong>Map layers</strong><label><input type="checkbox" checked={visibleLayers.hazards} onChange={() => setVisibleLayers((current) => ({ ...current, hazards: !current.hazards }))} /> Hazards</label><label><input type="checkbox" checked={visibleLayers.shelters} onChange={() => setVisibleLayers((current) => ({ ...current, shelters: !current.shelters }))} /> Help points</label><label><input type="checkbox" checked={visibleLayers.zones} onChange={() => setVisibleLayers((current) => ({ ...current, zones: !current.zones }))} /> Safety zones</label></div>}<ResidentMapSurface location={location} hazards={visibleHazards} shelters={visibleShelters} zones={visibleZones} selectedRoute={selectedRoute} ready={mapsReady} searchRequest={searchRequest} searchQuery={searchQuery} searchPoint={selectedPlace} zoomRequest={zoomRequest} streetViewRequest={streetViewRequest} onSearchResult={() => setRouteMessage("")} onSearchError={setRouteMessage} /></div>
